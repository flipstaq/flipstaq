...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .addTag('Internal User Management', 'Internal user management endpoints for admin operations')
    .build();
  ...
  console.log(`🚀 User Service (Internal) running on: http://localhost:${port}`);
  console.log(`📚 Internal API available at: http://localhost:${port}/internal`);
  console.log(`📚 Internal Swagger docs available at: http://localhost:${port}/api/docs`);
  console.log(`⚠️  This service should only be accessed via the API Gateway for external requests`);
}
