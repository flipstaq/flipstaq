  ...
  useEffect(() => {
    const fetchDocument = async () => {
      try {
        ...
        let response;
        try {
          response = await legalApi.getDocumentByType('cookies', language);
        } catch (primaryErr) {
          // If document not found and current language is Arabic, try English fallback
          if (language === 'ar') {
            try {
              response = await legalApi.getDocumentByType('cookies', 'en');
              if (response) {
              ...
              setError('Failed to load cookie policy document');
            }
          } else {
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {language === 'ar'
              ? 'سياسة الكوكيز غير متاحة.'
              : 'Cookie Policy is not available.'}
          </p>
        </div>
      </div>
    );
